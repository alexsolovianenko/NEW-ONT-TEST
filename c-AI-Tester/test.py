...
if __name__ == "__main__":
    ...
    practice_test_path = "c:\\Users\\solov\\ontario test\\NEW-ONT-TEST\\practice_test.txt"
    ...
    with open(practice_test_path, "r") as file:
        questions = file.read()
    ...
    section_markers = [
        (re.compile(r"\*\*Section 1: Thinking\*\*", re.IGNORECASE), "Thinking"),
        (re.compile(r"\*\*Section 2: Communication\*\*", re.IGNORECASE), "Communication"),
        (re.compile(r"\*\*Section 3: Knowledge\*\*", re.IGNORECASE), "Knowledge"),
        (re.compile(r"\*\*Section 4: Application\*\*", re.IGNORECASE), "Application"),
    ]

    def parse_sections(text, section_markers):
        section_map = {h: [] for h in section_headers}
        current_section = None
        for line in text.splitlines():
            line_strip = line.strip()
            for marker, header in section_markers:
                if marker.match(line_strip):
                    current_section = header
                    break
            else:
                if current_section and line_strip and not line_strip.startswith("**") and not line_strip.startswith("---"):
                    section_map[current_section].append(line_strip)
        return section_map
    ...
    question_map = parse_sections(questions_text, section_markers)
    ...
    for section in section_headers:
        if question_map[section]:  # Only show section if it has content
            ...
            for q in question_map[section]:
                # Remove old numbering (e.g., "1. ", "2. ", etc.) from the beginning
                q_clean = re.sub(r'^\d+\.\s*', '', q.strip())
                
                # Split multiple choice options onto separate lines
                if re.search(r'\b[a-e]\)', q_clean):
                    # Split by choice markers
                    parts = re.split(r'(?=[a-e]\))', q_clean)
                    ...
                    for part in parts[1:]:
                        if part.strip():
                            ...
                            pdf.multi_cell(0, 8, part.strip())
                    pdf.ln(5)  # More vertical space after multiple choice questions
                else:
                    pdf.multi_cell(0, 10, f"{question_number}. {q_clean}")
    ...
    if answers_text:
        # Use same section parsing for answers, but skip the first line ("**Answers**") and any note lines
        answer_lines = [l for l in answers_text.splitlines() if not l.strip().lower().startswith("**answers**") and not l.strip().startswith("*Note:")]
        answer_text_clean = "\n".join(answer_lines)
        answer_map = parse_sections(answer_text_clean, section_markers)
