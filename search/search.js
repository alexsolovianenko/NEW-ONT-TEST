...
function showSubjectSelection() {
    ...
    // Scroll within the card only, not the page
    const card = document.querySelector('.search-unified-card');
    if (card) {
        card.scrollTo({ top: subjectSelection.offsetTop - 100, behavior: 'smooth' });
    }
}
...
function showGradeOptions(subject) {
    ...
    if (grades) {
        ...
        // Scroll within the card only
        const card = document.querySelector('.search-unified-card');
        if (card) {
            card.scrollTo({ top: container.offsetTop - 100, behavior: 'smooth' });
        }
    } else {
...
function showSubjects(subject, grade) {
    ...
    if (subjects.length > 0) {
        ...
        subjects.forEach(topic => {
            ...
            topicBtn.addEventListener('click', function() {
                ...
                if (selectedTopics.length > 0) {
                    ...
                    const card = document.querySelector('.search-unified-card');
                    if (card) {
                        card.scrollTo({ top: submitButtonContainer.offsetTop - 100, behavior: 'smooth' });
                    }
                } else {
                    submitButtonContainer.style.display = "none";
                }
        ...
        const card = document.querySelector('.search-unified-card');
        if (card) {
            card.scrollTo({ top: subjectsContainer.offsetTop - 100, behavior: 'smooth' });
        }
    } else {
        const topicSection = document.createElement('div');
        topicSection.className = 'topic-section';
...
function submitAll() {
    ...
    // Map subject names to clean URL paths
    const subjectUrlMap = {
        "biology": "biology",
    ...
    if (urlPath) {
        // Navigate to clean URL: /subject/biology/, /subject/chemistry/, etc.
        window.location.href = `/subject/${urlPath}/`;
    } else {
